...
ELASTIC_PASSWORD="xxxxxxxxxxxxxxxxxxxxxxx"
query_time="now-1m"  # Last 1 minutes
DISCORD_CHANNEL_ID="xxxxxxxxxxxxxxxxxxxxxxx"
DISCORD_BOT_TOKEN="xxxxxxxxxxxxxxxxxxxxxxxxxx"
...
FROM_TIME=$(date -u -d "-1 minutes" +"%Y-%m-%dT%H:%M:%S.000Z")


# KIBANA_DISCOVER_URL="http://xxx.xxx.xxx.xxx:5601/app/discover#/?_g=(filters:!(),refreshInterval:(pause:!t,value:60000),time:(from:now-30m,to:now))&_a=(columns:!(),dataSource:(dataViewId:'$DATA_VIEW_ID',type:dataView),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,field:log.level,index:'$DATA_VIEW_ID',key:log.level,negate:!f,params:(query:INFO),type:phrase),query:(match_phrase:(log.level:INFO)))),hideChart:!f,interval:auto,query:(language:kuery,query:''),sort:!(!('@timestamp',desc)),viewMode:documents)"

KIBANA_DISCOVER_URL="http://xxx.xxx.xxx.xxx:5601/app/discover#/?_g=(filters:!(),refreshInterval:(pause:!t,value:60000),time:(from:'$FROM_TIME',to:'$TO_TIME'))&_a=(columns:!(),dataSource:(dataViewId:'$DATA_VIEW_ID',type:dataView),filters:!(('$state':(store:appState),meta:(alias:!n,disabled:!f,field:log.level,index:'$DATA_VIEW_ID',key:log.level,negate:!f,params:(query:INFO),type:phrase),query:(match_phrase:(log.level:INFO)))),hideChart:!f,interval:auto,query:(language:kuery,query:''),sort:!(!('@timestamp',desc)),viewMode:documents)"
...
if [ -z "$COUNT_RESULT_INFO" ]; then
  ...
  curl -X POST "https://discord.com/api/v10/channels/$DISCORD_CHANNEL_ID/messages" \
    -H "Authorization: Bot $DISCORD_BOT_TOKEN" \
    -H "Content-Type: application/json" \
    -d "{\"content\": \"\`\`\`\n$MESSAGE\n\`\`\`\", \"embeds\": [{\"title\": \"View in Kibana\", \"url\": \"$KIBANA_DISCOVER_URL\", \"color\": 16776960}]}"
  echo 1 > "$STATE_FILE"
fi